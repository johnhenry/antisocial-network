...
export const generatePost = async (
  ...
  try {
    ...
    const [post] = await db.create(TABLE_POST, {
      ...
      content, //k
      embedding: await embed(content), // k
      count: tokenize(content!).length, // k
      hash: hash(content!), // k
      source, // k
      target, // k
      mentions, // ?
      tools, // ?
      bibliography: conversation.concat(relevant), // k
    }) as Post[];
  ...
  try {
    ...
    if (content) {
      ...
      post = await generatePost({
        tools: inputTools && inputTools.length ? inputTools : undefined,
        target,
        streaming,
        source,
        bibliography,
