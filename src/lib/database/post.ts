...
export const generatePost = async (
  { tools, target, streaming = false, source, bibliography, depth, forward}: {
    tools?: string[];
    target?: Post;
    streaming?: boolean;
    source?: Agent;
...
const stringIdToAgent = async (id: string): Promise<Agent> => {
  const db = await getDB();
  try {
...
const processContent = (content: string, {
  ...
  return new Promise(async (rs, rj) => {
    ...
    try{
      ...
      //     params,
      //   );
      //   ({post, original, dehydrated, sequential, simultaneous} = result);
      //   if (post) {
      //     resolve(post);
      //   }
      // }
