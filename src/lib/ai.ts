...
export const PROMPTS_SUMMARIZE = {
  ...
  SUMMARY: `You are an advanced language model tasked with summarizing content. Read the provided content carefully and generate a concise and coherent summary that captures the key points and main ideas. The summary should be clear and informative, providing a comprehensive overview of the original content. Do not use quotes or spaces. Do not format the title in any way.`,
  LLM_NAMES: `You are an advanced language model tasked with giving a name to another language model based on it's prompt. Read the given system prompt carefully and generate five options for names of the LLM that uses the given prompts. Each name should be engaging and accurately reflects the main theme or key message of the prompt. The names should be is hyphenated and between 2 and 3 words long. Please provide only the comma-separated list of names and nothing else. Do not use quotes. Do not format the list in any way.`,
  LLM_PROMPT: `You are an advanced language model tasked with creating system prompts for other language models. Please read the provided content carefully and generate a prompt that instruct an LLM to behave in accordance with the content, mood, tone, and style of the given. Summarize the provided content into a clear and concise prompt that an LLM can follow effectively. Do not use quotes. Do not name the model in the prompt. Do not format the title in any way. The respone must begin with "You are an advanced language model "`,
